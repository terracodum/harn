...
Эталонное исправление хранится не одним скриптом, а цепочкой по требованиям:

```
task/solution/
├── solve.sh        # оркестратор: печатает "[solve] step R1", запускает solve_R1.sh, и так далее
├── solve_R1.sh     # атомарный патч для R1
└── solve_R3.sh     # атомарный патч для R3 (R2 — инвариант, шага нет)
```

Зачем: при лечении R3 модель не может «забыть» R1 и R2. Требования, которые уже сошлись (все их
...
- Self-healing перегенерирует весь бандл (тесты + шаги решения) целиком, до `limits.max_retries` раз;
  замораживание сошедшихся требований и `<focus>` сужают задачу модели, но контекст шага не
  пересчитывается по репозиторию после предыдущих шагов — модель видит их скрипты в промпте.
