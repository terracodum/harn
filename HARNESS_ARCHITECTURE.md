...
1. **Генерация эталонного решения (`solution/solve.sh`):**
   ...
   - Формируется минимальный скрипт модификации файлов (`solve.sh`), устраняющий проблему строго в рамках заявленных требований без лишнего рефакторинга.
...
4. **Учет расхода LLM:**
   - Каждый запрос логируется с фиксацией модели, длительности в секундах, количества входных и выходных токенов (для `evidence/llm_usage.json`).
...
5. **Петля самовосстановления (Self-Healing Loop):**
   - Если Base Run дал `reward != 0` или Oracle Run дал `reward != 1`, либо упала сборка Docker:
     ...
     - Отправляет ошибку в LLM с контекстом задачи и инструкцией: *"Исправь дефект в коде теста или solve.sh"*.
     - Повторяет цикл проверки (до 2-х итераций ретраев).
