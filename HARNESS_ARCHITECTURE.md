...
6. **Один кейс на требование:**  
   Бриф раскладывается на требования; каждое `bug/feature/change` требование становится отдельным, независимым бенчмарк-кейсом (`cases/R<i>/` с `task/`, `evidence/`, `result.json` по PROTOCOL.md), построенным на исходном репозитории. Инварианты и ограничения брифа защищаются в каждом кейсе (`pass_to_pass`, `anti_cheat`). Кейс, для которого не удалось получить эталон или тесты, помечается `case_failed` и всегда перечисляется в общем `result.json`; остальные кейсы не страдают.
...
1. **Генерация эталонного решения (`solution/solve.sh`):**
   - Модель определяет первопричину дефекта целевого требования кейса.
   - Решение возвращается как список структурных правок (`replace` / `create` / `delete`) исходного репозитория; харнесс проверяет до песочницы, что каждая правка применяется (`old` встречается ровно один раз), и детерминированно рендерит самодостаточный `solve.sh`.
   - Правки чинят только целевое требование; соседние требования брифа — отдельные кейсы.
...
4. **Учет расхода LLM:**
   ...
   - `evidence/case_spec.json` (в каждом кейсе): целевое требование, инварианты и ограничения кейса.
...
5. **Петля самовосстановления (Self-Healing Loop), отдельно для каждого кейса:**
   - Если Base Run дал `reward != 0` или Oracle Run дал `reward != 1`, либо упала сборка Docker:
     - Харнесс перехватывает `pytest.log` / `build.log` с трассировкой ошибки и отправляет их в LLM с контекстом кейса.
     - Бандл, изменивший правки без бизнес-падения на Oracle, отклоняется до песочницы: это дефект теста, а не решения.
     - Цикл проверки повторяется до `limits.max_retries` итераций.
   - **case_failed:** кейс, чей бандл не прошёл валидатор после repair-раундов, чей LLM-шаг упал или который не сошёлся за отведённые итерации, получает `case_status: "case_failed"` (в его `result.json` `status: "failed"`, `error`, `failed_stage`), а общий `result.json` прогона перечисляет его в `cases` и `limitations`. Прогон `ready` только когда все кейсы `ready`.
...
4. **Формирование `result.json`:**
   - В каждом кейсе `cases/R<i>/result.json`: поля PROTOCOL.md §2 (`status` ready | failed, `task_path`, `evidence_path`, `limitations`, `input_snapshot_sha256`) плюс `case_status`, `requirement_id`, `error`, `failed_stage`.
   - В корне `output_dir/result.json` — общий на прогон: те же протокольные поля (`task_path = null`, `cases_path = "cases"`), список `cases` со статусом каждого, счётчики `cases_ready` / `cases_failed`; `status: "ready"` только при всех успешных кейсах.
